...
import configparser
import datetime
from playbooks import extras
import argparse
import subprocess


DEBUG = [True if '-v' in sys.argv else False][0]
...
def management():
    ...
    parser.add_argument('-v','--null', action='store_true', help='show the version of ana')
    parser.add_argument('-git','--push', action='store_true', help='show the version of ana')
    ...
    if args.path:
        ...
        if args.change_dir:
            subprocess.call(f'bash {extras.main_directory}/playbooks/change_dir.sh {extras.main_directory}', shell=True)

        sys.exit(0)
    if args.push:
        subprocess.call(f'bash {extras.main_directory}/playbooks/git_push.sh {extras.main_directory}', shell=True)
        sys.exit(0)
